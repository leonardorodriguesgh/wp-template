...
<?php
   ...
   else 
   {
      ...
      try{
        ...
        $stmt = $conn->prepare(" INSERT INTO tb_newsletter (nm_contato,  nm_email) VALUES (:nome, :email) ");
        $stmt->bindValue(':nome',$nome);
        $stmt->bindValue(':email',$email);

        $stmt->execute();
        $gid = $conn->prepare(" SELECT MAX(id) FROM tb_newsletter ");
        $gid->execute();
        $id = $gid->fetchColumn();
      ...
      $msgToAdmin = "<b>Envio de Newsletter</b><br><br>";
      $msgToAdmin .= "<table border='0' cellspacing='7' cellpadding='7'>";
      $msgToAdmin .= "   <tr bgcolor='#F8F8F8'>
      ...
      //Texto e Assunto
      $mail->Subject  = "".$device." | Treinamento Lisieux: Newsletter Nº ".$id." "; // Assunto da mensagem
      $mail->Body = $msgToAdmin;
      $mail->AltBody  = "".$device." | Treinamento Lisieux: Newsletter Nº ".$id." ";
